...
// function. Len and Swap will usually be similar across 
// types and Less will hold the actual custom sorting logic. 
// In our case we want to sort in order of increasing string 
// length, so we use len(s[i]) and len(s[j]) here.
...
// sort by casting the original fruits slice to ByLength, and 
// then use sort.Sort on that typed slice.
func main() {
  fruits := []string{"peach", "banana", "kiwi", "ok"}
  sort.Sort(ByLength(fruits))
  fmt.Println(fruits)
}

// NOTE: By following this same pattern of creating a custom type, 
