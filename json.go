...
// First we’ll look at encoding basic data types to JSON 
// strings. Here are some examples for atomic values.
func main() {
  ...
  // The JSON package can automatically encode your custom data 
  // types. It will only include exported fields in the encoded 
  ...
  // Now let’s look at decoding JSON data into Go values. Here’s 
  // an example for a generic data structure.
  byt := []byte(`{"num":6.13, "strs":["a", "b"]}`)

  // We need to provide a variable where the JSON package can put 
  // the decoded data. This map[string]interface{} will hold a map 
  ...
  // programs and eliminating the need for type assertions when 
  ...
  // standard out. We can also stream JSON encodings directly to 
