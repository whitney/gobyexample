...
// goroutines. Here’s an example of using a blocking 
...
func main() {
  ...
  go worker(done)
  // do some other stuff here concurrently...

  // Block until we receive a notification from the worker 
  // on the channel.
  <-done
  // XXX IMPORTANT: If you removed the <- done line from this 
  // program, the program would exit before the worker even started.
}
