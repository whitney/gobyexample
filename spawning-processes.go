...
// site is implemented by spawning a pygmentize process from 
// a Go program. Let’s look at a few examples of spawning 
// processes from Go.
func main() {
  ...
  // .Output is another helper than handles the common case of running 
  ...
  // the usual if err != nil pattern for all of them. We also only collect 
  // the StdoutPipe results, but you could collect the StderrPipe in 
  ...
  // in one command-line string. If you want to spawn a full command 
  // with a string, you can use bash’s -c option:
