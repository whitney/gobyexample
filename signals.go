...
// shutdown when it receives a SIGTERM, or a command-line tool to 
// stop processing input if it receives a SIGINT. Here’s how to 
// handle signals in Go with channels.
func main() {
  // Go signal notification works by sending os.Signal values on 
  // a channel. We’ll create a channel to receive these notifications 
...
// By typing ctrl-C (which the terminal shows as ^C) we can send 
// a SIGINT signal, causing the program to print interrupt and 
// then exit.
